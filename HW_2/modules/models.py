...
class CatBoostDirect(BaseModel):
    ...
    def fit(
        ...
        """
        self.models = []
        tr_X_index, tr_y_index = features_targets__train_idx(
            id_column=train_data[id_col],
            series_length=len(train_data),
            model_horizon=self.model_horizon,
            history_size=self.history,
        )
        va_X_index, va_y_index = features_targets__train_idx(
            id_column=val_data[id_col],
            series_length=len(val_data),
            model_horizon=self.model_horizon,
            history_size=self.history,
        ...
        steps = self.horizon // self.model_horizon
        for step in range(steps):
            shift = step * self.model_horizon
            tr_y_index_s = tr_y_index + shift
            va_y_index_s = va_y_index + shift

            tr_ok = (tr_y_index_s < len(train_data)).all(axis=1)
            va_ok = (va_y_index_s < len(val_data)).all(axis=1)

            tr_X_index_s = tr_X_index[tr_ok]
            tr_y_index_s = tr_y_index_s[tr_ok]
            va_X_index_s = va_X_index[va_ok]
            va_y_index_s = va_y_index_s[va_ok]
            tr_X, tr_y, cat_idx = get_features_df_and_targets(
                train_data,
                tr_X_index_s,
                tr_y_index_s,
                id_column=id_col,
            ...
            va_X, va_y, va_probe = get_features_df_and_targets(
                val_data,
                va_X_index_s,
                va_y_index_s,
                id_column=id_col,
            ...
            model.fit(
                cb.Pool(tr_X, tr_y, cat_features=cat_idx),
                eval_set=cb.Pool(va_X, va_y, cat_features=cat_idx),
                use_best_model=True,
                plot=False,
            )
    ...
    def predict(self, test_data, id_col="sensor_id", timestamp_col="timestamp", value_col="value"):
            ...
            """
            X_index, y_index = features__test_idx(
                id_column=test_data[id_col],
                series_length=len(test_data),
                model_horizon=self.model_horizon,
                history_size=self.history,
            ...
            for step in range(steps):
                shift = step * self.model_horizon
                y_index_s = y_index + shift
                ok = (y_index_s < len(test_data)).all(axis=1)
                X_index_s = X_index[ok]
                y_index_s = y_index_s[ok]

                X, pro_1, pro_2 = get_features_df_and_targets(
                    test_data, X_index_s, y_index_s,
                    id_column=id_col, date_column=timestamp_col, target_column=value_col
                )
                preds = self.models[step].predict(X).reshape(-1, self.model_horizon)
                idx_flat = y_index_s.flatten()
                part = test_data.iloc[idx_flat][[id_col, timestamp_col]].copy()
                part["predicted_value"] = preds.flatten()
                preds_list.append(part)

            test_data = pd.concat(preds_list, ignore_index=True)
            counts = test_data.groupby(id_col).size()
            # Проверки
            if not (counts == self.horizon).all():
                print("Ошибка: не у всех рядов ровно horizon предсказаний!")
                print(counts)
                raise ValueError("Некорректный размер горизонта в Direct predict()")
            duplicates = test_data.duplicated(subset=[id_col, timestamp_col]).sum()
            if duplicates > 0:
                print(f"Найдено {duplicates} дубликатов (id, timestamp)!")
                raise ValueError("Дубликаты предсказаний в Direct predict()")
